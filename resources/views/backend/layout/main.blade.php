<!DOCTYPE html>
<html dir="@if (Config::get('app.locale') == 'ar' || optional($general_setting)->is_rtl) {{ 'rtl' }} @endif">
...
<head>
    ...
    @if (!config('database.connections.saleprosaas_landlord'))
        ...
        <link rel="stylesheet" href="<?php echo asset('css/custom-' . optional($general_setting)->theme); ?>" type="text/css" id="custom-style">

        @if (Config::get('app.locale') == 'ar' || optional($general_setting)->is_rtl)
            <!-- RTL css -->
    ...
    @else
        <link rel="icon" type="image/png"
            href="{{ url('../../logo', optional($general_setting)->site_logo) }}" />
        <title>{{ optional($general_setting)->site_title }}</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="robots" content="all,follow">
    ...
    <nav class="side-navbar shrink d-print-none">
        <span class="brand-big">
            @if (optional($general_setting)->site_logo)
                <a href="{{ url('/') }}"><img src="{{ url('logo', optional($general_setting)->site_logo) }}"
                        width="115"></a>
            @else
                <a href="{{ url('/') }}">
                    <h1 class="d-inline">{{ optional($general_setting)->site_title }}</h1>
                </a>
            @endif
        </span>
    ...
    <div class="page">
                    ...
                    <ul class="nav-menu list-unstyled d-flex flex-md-row align-items-md-center">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                @if ($category_permission_active)
                                    <li class="dropdown-item"><a data-toggle="modal"
                                            data-target="#category-modal">{{ __('file.Add Category') }}</a></li>
                                @endif
                                <?php
                                ...
                                @if ($return_add_permission_active)
                                    <li class="dropdown-item"><a href="#" data-toggle="modal"
                                            data-target="#add-sale-return"> {{ trans('file.Add Return') }}</a></li>
                                @endif
                                <?php
                        ...
                        @if (config('database.connections.saleprosaas_landlord'))
                            <li class="nav-item"><a target="_blank"
                                    ...
                                    data-toggle="tooltip" title="{{ trans('file.Renew Subscription') }}"><i
        ...
        <footer class="main-footer">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-12">
                        <p>&copy; {{ optional($general_setting)->site_title }} | {{ trans('file.Developed') }}
                            {{ trans('file.By') }} <span
                                class="external">{{ optional($general_setting)->developed_by }}</span> | V
                            {{ env('VERSION') }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <button type="submit"
                                class="btn btn-primary category-submit-btn">{{ trans('file.submit') }}</button>
                        </div>
                    </div>
                    {{ Form::close() }}
                </div>
        ...
        <div id="customer-group-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <div role="document" class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 id="exampleModalLabel" class="modal-title">{{ trans('file.Customer Group Report') }}
                        </h5>
                        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span
                                aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                    </div>
