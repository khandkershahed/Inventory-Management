        ...
        <table id="report-table" class="table table-hover">
            ...
            <tbody>
                @foreach($lims_purchase_data as $key => $purchase_data)
                    ...
                    <tr>
                        <td>{{$key}}</td>
                        <td>{{date(optional($general_setting)->date_format, strtotime($purchase_data->created_at->toDateString())) . ' '. $purchase_data->created_at->toTimeString()}}</td>
                        <td>{{$purchase_data->reference_no}}</td>
                        <td>{{$supplier->name .' (' .$supplier->phone_number . ')'}}</td>
                        <td>{{number_format((float)$purchase_data->grand_total, optional($general_setting)->decimal, '.', '')}}</td>
                        <td>{{number_format((float)$returned_amount, optional($general_setting)->decimal, '.', '')}}</td>
                        @if($purchase_data->paid_amount)
                        <td>{{number_format((float)$purchase_data->paid_amount, optional($general_setting)->decimal, '.', '')}}</td>
                        @else
                        <td>{{number_format(0, optional($general_setting)->decimal, '.', '')}}</td>
                        @endif
                        <td>{{number_format((float)($purchase_data->grand_total - $returned_amount - $purchase_data->paid_amount), optional($general_setting)->decimal, '.', '')}}</td>
                    </tr>
                    @endif
                @endforeach
            </tbody>
